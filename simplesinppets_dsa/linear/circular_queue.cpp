...
class CircularQueue{
	...
	public:
		CircularQueue(){             // constructor function
		...
		bool isFull(){
			if((rear+1)%5 == front)                 // since the size is 5 and the position starts from 0
				return true;
			else
				return false;
		}
		...
		void enqueue(int val){            //first check if the queue is full or not
			if(isFull()){
				...
				rear = (rear+1)%5;                  //default process is to first increment the rear value and the assign it
				arr[rear] = val;
				itemCount++;
			}
		}
		...
		int count(){
			return (itemCount);      // standard math stuff
		}
